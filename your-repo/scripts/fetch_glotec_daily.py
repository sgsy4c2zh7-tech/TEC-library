...
def choose_3hourly(items_for_day: list[dict], day_utc: datetime) -> list[dict]:
    ...
    If NOAA cadence is e.g. every 15 min, this selects closest available in that day.
...
def prune_old_day_folders(out_dir: str, keep_days: int, dry_run: bool) -> None:
    ...
    # Example: keep_days=30 -> keep cutoff..today inclusive (30 days)
...
def main() -> int:
    ...
    if not items_for_day:
        ...
        # Still prune old folders even if fetch failed
    ...
    safe_write_bytes(index_path, json.dumps(meta, ensure_ascii=False, indent=2).encode("utf-8"))
